...
import PRG
from PRG import PRGCamera
from PRG.exceptions import PRGError
...
class PRGDevice:
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry) -> None:
        ...
        self.PRG_capabilities: dict[str, Any] | None = None
    ...
    async def async_setup(self) -> None:
        ...
        self.PRG_capabilities = await self.device.get_capabilities()
        ...
        if not self.profiles:
            raise PRGError("No camera profiles found")
        ...
        # Bind the listener to the PRGDevice instance since
        ...
        # the PRGDevice instance is stored in hass.data
    ...
    async def async_check_date_and_time(self) -> None:
        ...
        LOGGER.debug("%s: Setting up the PRG device management service", self.name)
        device_mgmt = await self.device.create_devicemgmt_service()
        system_date = dt_util.utcnow()
    ...
    async def async_get_device_info(self) -> DeviceInfo:
        ...
        try:
            ...
            LOGGER.debug(
                "Couldn't get network interfaces from PRG device '%s'. Error: %s",
                self.name,
                fault,
            )
    ...
    async def async_start_events(self):
        ...
        with suppress(*GET_CAPABILITIES_EXCEPTIONS, XMLParseError):
            PRG_capabilities = self.PRG_capabilities or {}
            pull_point_support = (PRG_capabilities.get("Events") or {}).get(
                "WSPullPointSupport"
            )
    ...
    async def async_get_profiles(self) -> list[Profile]:
        ...
        try:
            ...
            LOGGER.debug(
                "%s: Could not get profiles from PRG device", self.name, exc_info=True
            )
            raise
        ...
        for key, PRG_profile in enumerate(result):
            # Only add H264 profiles
            if (
                not PRG_profile.VideoEncoderConfiguration
                or PRG_profile.VideoEncoderConfiguration.Encoding != "H264"
            ):
                continue
            ...
            profile = Profile(
                ...
                PRG_profile.Name,
                Video(
                    PRG_profile.VideoEncoderConfiguration.Encoding,
                    Resolution(
                        PRG_profile.VideoEncoderConfiguration.Resolution.Width,
                        PRG_profile.VideoEncoderConfiguration.Resolution.Height,
                    ),
                ),
            )

            # Configure PTZ options
            if self.capabilities.ptz and PRG_profile.PTZConfiguration:
                profile.ptz = PTZ(
                    PRG_profile.PTZConfiguration.DefaultContinuousPanTiltVelocitySpace
                    is not None,
                    PRG_profile.PTZConfiguration.DefaultRelativePanTiltTranslationSpace
                    is not None,
                    PRG_profile.PTZConfiguration.DefaultAbsolutePantTiltPositionSpace
                    is not None,
                )
            ...
            if self.capabilities.imaging and PRG_profile.VideoSourceConfiguration:
                profile.video_source_token = (
                    PRG_profile.VideoSourceConfiguration.SourceToken
                )
    ...
    async def async_set_imaging_settings(
        ...
        """Set an imaging setting on the PRG imaging service."""
        # The Imaging Service is defined by PRG standard
...
def get_device(
    ...
    """Get PRGCamera instance."""
    return PRGCamera(
        host,
        port,
        username,
        password,
        f"{os.path.dirname(PRG.__file__)}/wsdl/",
        no_cache=True,
    )
