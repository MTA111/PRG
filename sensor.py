...
async def async_setup_entry(
    ...
    entities = {
        event.uid: PRGSensor(event.uid, device)
    ...
    for entry in er.async_entries_for_config_entry(ent_reg, config_entry.entry_id):
        if entry.domain == "sensor" and entry.unique_id not in entities:
            entities[entry.unique_id] = PRGSensor(entry.unique_id, device, entry)
    ...
    def async_check_entities() -> None:
        ...
        if not (missing := uids_by_platform.difference(entities)):
            return
        new_entities: dict[str, PRGSensor] = {
            uid: PRGSensor(uid, device) for uid in missing
        }
        if new_entities:
...
class PRGSensor(PRGBaseEntity, RestoreSensor):
    """Representation of a PRG sensor event."""

    _attr_should_poll = False

    def __init__(
        self, uid, device: PRGDevice, entry: er.RegistryEntry | None = None
    ) -> None:
