...
from PRG.util import is_auth_error, stringify_PRG_error
...
CONF_MANUAL_INPUT = "Manually configure PRG device"


def wsdiscovery() -> list[Service]:
    """Get PRG Profile S devices from network."""
    discovery = WSDiscovery(ttl=4)
    try:
        discovery.start()
        return discovery.searchServices(
            scopes=[Scope("PRG://www.PRG.org/Profile/Streaming")]
        )
    finally:
        discovery.stop()
...
async def async_discovery(hass: HomeAssistant) -> list[dict[str, Any]]:
    ...
    LOGGER.debug("Starting PRG discovery")
    ...
    for service in services:
        ...
        for scope in service.getScopes():
            ...
            if scope_str.lower().startswith("PRG://www.PRG.org/name"):
                device[CONF_NAME] = scope_str.split("/")[-1]
            if scope_str.lower().startswith("PRG://www.PRG.org/hardware"):
                device[CONF_HARDWARE] = scope_str.split("/")[-1]
            if scope_str.lower().startswith("PRG://www.PRG.org/mac"):
...
class PRGFlowHandler(ConfigFlow, domain=DOMAIN):
    """Handle a PRG config flow."""
    ...
    ) -> PRGOptionsFlowHandler:
        """Get the options flow for this handler."""
        return PRGOptionsFlowHandler(config_entry)

    def __init__(self):
        """Initialize the PRG config flow."""
        self.device_id = None
        self.devices = []
        self.PRG_config = {}
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            ...
            self.PRG_config = entry_data | user_input
            errors, description_placeholders = await self.async_setup_profiles(
                configure_unique_id=False
            )
            if not errors:
                return self.async_update_reload_and_abort(entry, data=self.PRG_config)
    ...
    async def async_step_device(self, user_input=None):
        ...
        if LOGGER.isEnabledFor(logging.DEBUG):
            LOGGER.debug("Discovered PRG devices %s", pformat(self.devices))
    ...
    async def async_step_configure(
        ...
        if user_input:
            self.PRG_config = user_input
            errors, description_placeholders = await self.async_setup_profiles()
            if not errors:
                title = f"{self.PRG_config[CONF_NAME]} - {self.device_id}"
                return self.async_create_entry(title=title, data=self.PRG_config)

        def conf(name, default=None):
            return self.PRG_config.get(name, default)
        return self.async_show_form(
            step_id="configure",
            data_schema=vol.Schema(
                {
    ...
    async def async_setup_profiles(
        ...
        """Fetch PRG device profiles."""
        if LOGGER.isEnabledFor(logging.DEBUG):
            LOGGER.debug(
                "Fetching profiles from PRG device %s", pformat(self.PRG_config)
            )

        device = get_device(
            self.hass,
            self.PRG_config[CONF_HOST],
            self.PRG_config[CONF_PORT],
            self.PRG_config[CONF_USERNAME],
            self.PRG_config[CONF_PASSWORD],
        )
        ...
        try:
            ...
            if not self.device_id:
                try:
                    ...
                    LOGGER.debug(
                        ...
                        self.PRG_config[CONF_NAME],
                        stringify_PRG_error(fault),
                    )
            if not self.device_id:
            ...
            if configure_unique_id:
                ...
                self._abort_if_unique_id_configured(
                    updates={
                        CONF_HOST: self.PRG_config[CONF_HOST],
                        CONF_PORT: self.PRG_config[CONF_PORT],
                        CONF_NAME: self.PRG_config[CONF_NAME],
                        CONF_USERNAME: self.PRG_config[CONF_USERNAME],
                        CONF_PASSWORD: self.PRG_config[CONF_PASSWORD],
                    }
                )
            ...
            LOGGER.debug(
                "%s: No PRG service found at %s:%s",
                self.PRG_config[CONF_NAME],
                self.PRG_config[CONF_HOST],
                self.PRG_config[CONF_PORT],
                exc_info=True,
            )
            return {CONF_PORT: "no_PRG_service"}, {}
        except Fault as err:
            stringified_error = stringify_PRG_error(err)
            ...
            return {"base": "PRG_error"}, description_placeholders
        except GET_CAPABILITIES_EXCEPTIONS as err:
            LOGGER.debug(
                "%s: Could not determine camera capabilities: %s",
                self.PRG_config[CONF_NAME],
                stringify_PRG_error(err),
                exc_info=True,
            )
            return {"base": "PRG_error"}, {"error": stringify_PRG_error(err)}
        else:
            if not any(
...
class PRGOptionsFlowHandler(OptionsFlow):
    """Handle PRG options."""

    def __init__(self, config_entry: ConfigEntry) -> None:
        """Initialize PRG options flow."""
        self.config_entry = config_entry
        self.options = dict(config_entry.options)

    async def async_step_init(self, user_input=None):
        """Manage the PRG options."""
        return await self.async_step_PRG_devices()

    async def async_step_PRG_devices(self, user_input=None):
        """Manage the PRG devices options."""
        if user_input is not None:
