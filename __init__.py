...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up PRG from a config entry."""
    ...
    try:
        ...
        if is_auth_error(err):
            raise ConfigEntryAuthFailed(
                f"Auth Failed: {stringify_PRG_error(err)}"
            ) from err
        raise ConfigEntryNotReady(
            f"Could not connect to camera: {stringify_PRG_error(err)}"
        ) from err
    except PRGError as err:
        await device.device.close()
        raise ConfigEntryNotReady(
            f"Could not setup camera {device.device.host}:{device.device.port}: {stringify_PRG_error(err)}"
        ) from err
    except TransportError as err:
        await device.device.close()
        stringified_PRG_error = stringify_PRG_error(err)
        if err.status_code in (
            HTTPStatus.UNAUTHORIZED.value,
            HTTPStatus.FORBIDDEN.value,
        ):
            raise ConfigEntryAuthFailed(
                f"Auth Failed: {stringified_PRG_error}"
            ) from err
        raise ConfigEntryNotReady(
            f"Could not setup camera {device.device.host}:{device.device.port}: {stringified_PRG_error}"
        ) from err
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    device: PRGDevice = hass.data[DOMAIN][entry.unique_id]

    if device.capabilities.events and device.events.started:
        try:
            await device.events.async_stop()
        except (PRGError, Fault, RequestError, TransportError):
...
async def _get_snapshot_auth(device: PRGDevice) -> str | None:
...
async def async_populate_snapshot_auth(
    hass: HomeAssistant, device: PRGDevice, entry: ConfigEntry
) -> None:
